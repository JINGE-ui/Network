...
void TCPRdtSender::receive(const Packet& ackPkt) {
	...
	if (checkSum == ackPkt.checksum) {
		if ((ackPkt.acknum - sendbase >= 0 && ackPkt.acknum - sendbase <= TCP_Winsize) || (ackPkt.acknum + 1 + TCP_seqnum - sendbase <= TCP_Winsize)) {
			waitingState = false;
			y = -1;
			num = 0;
			if ((ackPkt.acknum + 1)% TCP_seqnum == nextseqnum) {
				pns->stopTimer(SENDER, sendbase);		//关闭定时器
			}
			else {
				pns->stopTimer(SENDER, sendbase);		//关闭定时器
				pns->startTimer(SENDER, Configuration::TIME_OUT, (ackPkt.acknum + 1)% TCP_seqnum);
			}
			sendbase = (ackPkt.acknum + 1)% TCP_seqnum;
